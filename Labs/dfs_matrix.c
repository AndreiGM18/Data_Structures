...
void
visit(int node, int** color, int** t_desc, int *time)
{
	++(*time);
...
dfs_matrix_graph(matrix_graph_t* mg,
                 ...
                 int* t_tin)
{
	init_dfs(mg, &color, &t_desc, &t_tin);

	int time = 0;

	stack *st = st_create(sizeof(int));

	visit(node, &color, &t_desc, &time);
	st_push(st, &node);
	...
	while (!st_is_empty(st)) {
		...
		int v = -1;
		for (int i = 0; i < mg->nodes; ++i) {
			if (color[i] == -1 && mg->matrix[top][i] == 1) {
				v = i;
				break;
			}
		}
		if (v != -1) {
			visit(v, &color, &t_desc, &time);
			st_push(st, &v);
		} else {
			++time;
			t_tin[top] = time;
