...
treap_tree_t* treap_create(size_t data_size, int (*cmp)(void*, void*))
{
	...
	// TODO: Creati un treap si completati toate campurile structurii
	treap_tree_t *treap = (treap_tree_t *)malloc(sizeof(*treap));
	
	treap->cmp = cmp;
	treap->data_size = data_size;
	treap->root = NULL;
...
{
	// TODO: Eliberati memoria folosita
	if (!(*node))
		return;
...
static void __treap_insert(treap_node_t** node,
						...
						int (*cmp)(void*, void*))
{
	// TODO: Inserati recursiv in arbore si reechilibrati arborele
	if (!(*node)) {
		treap_node_t *new = __treap_node_create(data, data_size);
		*node = new;
						...
						void (*free_data)(void *))
{
	...
	if (cmp(data, (*node)->data) > 0) {
		...
		*node = (*node)->right;
		...
		__rotate_right(node);
		__treap_delete(node, data, cmp, free_data);
	} else {
		__rotate_left(node);
		__treap_delete(node, data, cmp, free_data);
	}
...
{
	// TODO: Cautati valoarea data in arbore
	if (!node)
		return NULL;
...
void __treap_ascending_nodes(treap_node_t *node, int *keys, int *num_keys)
{
	if (!node)
		return;
		
	__treap_ascending_nodes(node->left, keys, num_keys);
	keys[(*num_keys)] = *(int *)node->data;
	++(*num_keys);
	__treap_ascending_nodes(node->right, keys, num_keys);
}
...
{
	// TODO: Parcurgeti arborele recursiv si stocati in keys datele acestora
	__treap_ascending_nodes(node, keys, num_keys);
}
