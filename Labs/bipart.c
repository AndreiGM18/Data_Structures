...
void
bfs_list_graph(list_graph_t* lg, int node, int** state, int **level, int *bipart)
{
...
void print_bipartite(list_graph_t *graph)
{
	...
	int* level = (int *)malloc(graph->nodes * sizeof(int));
	DIE(!level, "malloc level failed");

	init_bfs(graph, &state, &level);
	int bipart = 1;

	for (int i = 0; i < graph->nodes; ++i) {
		if (state[i] == -1) {
			bfs_list_graph(graph, i, &state, &level, &bipart);
		}
		if (!bipart) {
