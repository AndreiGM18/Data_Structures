...
unsigned int check_func(b_node_t *root, int level, int *leaf_level)
{
	if (!root)
		return 1;
	...
	return check_func(root->left, level + 1, leaf_level) &&
		check_func(root->right, level + 1, leaf_level);
}

unsigned int check(b_node_t *root)
{
	int level = 0, leaf_level = 0;
	return check_func(root, level, &leaf_level);
}
...
void b_tree_level_leaves(b_tree_t *b_tree)
{
	...
	(__b_tree_level_leaves(b_tree->root) == 1) ? printf("YES\n") : printf("NO\n");
}
