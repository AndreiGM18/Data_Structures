...
st_peek(stack_t *st)
{
    ...
    return (get_nth_node(st->list, st->list->size))->data;
}
...
st_pop(stack_t *st)
{
	...
	ll_node_t *removed = ll_remove_nth_node(st->list, st->list->size);
	free(removed->data);
	free(removed);
}
...
st_clear(stack_t *st)
{
	...
	for (unsigned int i = 0; i < st->list->size; ++i) {
		ll_node_t *removed = ll_remove_nth_node(st->list, st->list->size);
		free(removed->data);
		free(removed);
	}
...
/*
 * Functia verifica dacă a si b sunt paranteze corespunzatoare
 */
int
check_parant(char a, char b)
{
	if (a == '{' && b == '}')
		return 1;
	else if (a == '[' && b == ']')
...
{
    int i = 0;
	stack_t *st = st_create(MAX_STRING_SIZE);

	while (brackets[i]) {
		if ((brackets[i] == '{') || (brackets[i] == '[') || (brackets[i] == '('))
			st_push(st, &brackets[i]);
		else if ((brackets[i] == '}') || (brackets[i] == ']') || (brackets[i] == ')')) {
			if (st_is_empty(st) || !check_parant(*((char *)st_peek(st)), brackets[i])) {
				st_free(st);
				return 0;
			}
			else
