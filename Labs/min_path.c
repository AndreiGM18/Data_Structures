...
void
bfs_list_graph(list_graph_t* lg, int node, int** state, int **d, int** parent)
{
...
void print_min_path(list_graph_t *graph, int src, int dest)
{
	...
	int* d = (int *)malloc(graph->nodes * sizeof(int));
	DIE(!d, "malloc d failed");
	int* parent = (int *)malloc(graph->nodes * sizeof(int));
	DIE(!parent, "malloc parent failed");

	init_bfs(graph, &state, &d, &parent);

	bfs_list_graph(graph, src, &state, &d, &parent);
	...
	if (d[dest] == -1) {
		...
		int v = dest;
		while (v != src) {
			ll_add_nth_node(list, 0, &v);
			v = parent[v];
		}
		ll_add_nth_node(list, 0, &src);
		ll_print_int(list);
	}
