...
bst_node_t* min_value_node(bst_node_t* node)
{
	...
	int (*cmp)(const void *, const void *))
{
	...
	if (rc < 0) {
        ...
        bst_node_t *temp = min_value_node(bst_node->right);
 
        memcpy(bst_node->data, temp->data, data_size);

        bst_node->right = __bst_tree_remove(bst_node->right, temp->data, data_size, cmp);
	}
