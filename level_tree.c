...
void print_at_level_n(b_node_t* root, int level, int n)
{
	if (!root)
		return;

	print_at_level_n(root->left, level + 1, n);
	print_at_level_n(root->right, level + 1, n);
...
{
	print_at_level_n(b_tree->root, 0, n);
