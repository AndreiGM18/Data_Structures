...
{
	init_bfs(lg, &color, &parent);

	queue_t *q = q_create(sizeof(int), MAX_SIZE);
	...
	while(!q_is_empty(q)) {
		...
		ll_node_t *it = lg->neighbors[v]->head;
		for (uint i = 0; i < lg->neighbors[v]->size; ++i) {
			int u = *(int *)it->data;
			if (color[u] == -1) {
				color[u] = 0;
				parent[u] = v;
